...
### 特性
1. 支持常见的GM特性，如CC-7、CC-10、note_off事件、lyrics事件等
2. 支持新旧Java版和基岩版游戏，对各个版本做了语法和音效的适配
3. 支持加载标准.lrc文件或加载MIDI内嵌歌词以在游戏中显示
4. 支持在.mcstructure文件中预添加一些指令或结构
5. 支持压缩游戏指令条数等......
...
```
每个键都对应一个列表（当忽略该乐器时需将列表替换为null），列表内又有若干个列表（也就是一个MIDI乐器可以对应多个音效）。  
列表中，元素依次为游戏音效ID、音量倍率、音调偏移、相对延迟时间（指距离上个音效的时间，单位为ms）。